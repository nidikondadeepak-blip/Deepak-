...
export class Game {
  ...
  start() {
    ...
    // initial camera guess from spawn
    const meSnap = this.room.players.find((p) => p.id === this.meId);
    void meSnap;
  ...
  _onCanvasClick = () => {
    if (!this.locked && !this.isTouch()) this.canvas.requestPointerLock?.();
  };
  ...
  _onMouseUp = (e) => { if (e.button === 0) this.firing = false; };
  ...
  tryFire() {
    ...
    this.lastFireAt = now;
    net.emit('fire', { yaw: this.yaw, pitch: this.pitch, weapon: this.weapon });
    // optimistic feedback
    const rec = this.players.get(this.meId);
    if (rec) {
  ...
  onSnapshot(snap) {
    ...
    for (const p of snap.players) {
      ...
      if (!rec) {
        ...
        rec = { ninja, marker, rx: p.x, ry: p.y, rz: p.z, yaw: p.yaw, hp: p.hp, alive: true, firstSeen: true, dead: false };
  ...
  updateCamera(dt, time) {
    ...
    // desired camera pos: behind + above + shoulder offset
    const shyaw = this.yaw - Math.PI / 2;
    const shx = -Math.sin(shyaw) * 0.0, shz = -Math.cos(shyaw) * 0.0;
    void shx; void shz;
    const headY = py + 1.7;
  ...
  loop(now) {
    ...
    // interpolate players toward latest snapshot
    const k = 1 - Math.exp(-dt * 14);
    const ky = 1 - Math.exp(-dt * 18);
    for (const [, rec] of this.players) {
      const t = rec.target;
      if (!t) continue;
      // snap on teleport-ish jumps (spawn / barrage)
      const dJump = Math.hypot(t.x - rec.rx, t.z - rec.rz);
      if (dJump > 12) { rec.rx = t.x; rec.ry = t.y; rec.rz = t.z; }
      else {
        rec.rx += (t.x - rec.rx) * k;
        rec.ry += (t.y - rec.ry) * Math.min(1, k * 1.4);
        rec.rz += (t.z - rec.rz) * k;
      }
