...
// ============================================================================
import { net, setNetBackend } from './net.js';
import { LocalNet } from './localnet.js';
import { getConfigPayload } from '../../server/constants.js';
import { listMaps, getMapLayout } from '../../server/mapLayout.js';
import { Game } from './game.js';

// Offline demo mode (GitHub Pages): no socket.io server -> run the real
// simulation locally in the browser. Online mode is untouched.
const OFFLINE = typeof io === 'undefined';
...
async function startGameFromRoom(room) {
  ...
  try {
    const layout = OFFLINE
      ? getMapLayout(room.mapId)
      : await fetch(`api/map/${room.mapId}`).then((r) => {
        if (!r.ok) throw new Error('map failed to load');
        return r.json();
      });
    if (!layout) throw new Error('map failed to load');
    lastRoom = room;
    showScreen('game');
...
async function boot() {
  ...
  if (OFFLINE) {
    ...
    // on LocalNet (same GameRoom code the server uses).
    $('loading-msg').textContent = 'Loading offline demo…';
    CONFIG = getConfigPayload();
    MAPS = listMaps();
    setNetBackend(new LocalNet());
    const tag = document.querySelector('.tagline');
    if (tag) tag.textContent = 'OFFLINE DEMO · you vs 7 bot ninjas · 200 HP · ninja weapons only';
  } else {
    try {
  ...
  document.addEventListener('pointerlockchange', () => {
    if (inGame && game && !game.matchOver && !document.pointerLockElement && !game.isTouch() && !game.fallbackLook) {
      const pm = $('pause-menu');
